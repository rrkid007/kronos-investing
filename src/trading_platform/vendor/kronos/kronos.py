...
# PATCH(trading-platform): package-relative import instead of sys.path hack;
# optional return_samples kwarg threaded through predict() so callers can get
# per-sample forecast paths instead of the internally-averaged mean path.
import numpy as np
import pandas as pd
import torch
...
def auto_regressive_inference(tokenizer, model, x, x_stamp, y_stamp, max_context, pred_len, clip=5, T=1.0, top_k=0, top_p=0.99, sample_count=5, verbose=False, return_samples=False):  # PATCH(trading-platform)
    with torch.no_grad():
        ...
        preds = z.cpu().numpy()
        if return_samples:  # PATCH(trading-platform): (batch, sample_count, seq, dim)
            return preds
...
class KronosPredictor:
    ...
    def generate(self, x, x_stamp, y_stamp, pred_len, T, top_k, top_p, sample_count, verbose, return_samples=False):  # PATCH(trading-platform)
        ...
        if return_samples:  # PATCH(trading-platform): (batch, sample_count, pred_len, dim)
            preds = preds[:, :, -pred_len:, :]
        else:
            preds = preds[:, -pred_len:, :]
        return preds

    def predict(self, df, x_timestamp, y_timestamp, pred_len, T=1.0, top_k=0, top_p=0.9, sample_count=1, verbose=True, return_samples=False):  # PATCH(trading-platform)
        ...
        if return_samples:  # PATCH(trading-platform): ndarray (sample_count, pred_len, dim)
            return preds
